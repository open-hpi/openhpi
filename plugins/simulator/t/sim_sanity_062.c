...
static int inject_event(char *plugin_name) {
    ...
    *txtptr = '\0';
    ...
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_TYPE, SAHPI_CTRL_TYPE_ANALOG);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    ...
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_ANALOG_MIN, 0);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    }
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_ANALOG_MAX, 10);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    }
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_ANALOG_DEFAULT, 5);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
