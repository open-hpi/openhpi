...
int main(int argc, char **argv) 
{
	...
	/************************** 
	 * Test : snmp_bc_set_sensor_thresholds()
	 **************************/
	 ...
	 * Test   
	 **************************/
	err = snmp_bc_set_sensor_thresholds((void *)h->hnd, id, sid, NULL);
	checkstatus(&err, &expected_err, &testfail);
	...
	err = snmp_bc_set_sensor_thresholds((void *)h->hnd, 5000, sid, &thres);
	checkstatus(&err, &expected_err, &testfail);
	...
	err = snmp_bc_set_sensor_thresholds((void *)h->hnd, id, 5000, &thres);
	checkstatus(&err, &expected_err, &testfail);
