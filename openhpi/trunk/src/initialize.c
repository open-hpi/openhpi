...
int init(void)
{
        ...
        if ( _initialize(TRUE) == SA_ERR_HPI_ERROR) return SA_ERR_HPI_ERROR;
...
int _initialize(int daemon)
{
        ...
        oh_get_global_param(&my_global_param);
                ...
                dbg("### We ARE Going to Check if start infrastructure threads"
...
//        exit(0);
//} else {
//        dbg("Async event log creation Succeeded");
//}
                ...
                */
                ...
                oh_threaded_start();
                dbg("### We ARE Starting infrastructure threads"
                      " in _initialize() my_global_param.u.daemon_mode[%d]"
                      " daemon[%d]###\n",
                      my_global_param.u.daemon_mode, daemon);
