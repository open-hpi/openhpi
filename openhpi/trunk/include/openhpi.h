...
struct oh_domain {
	...
	/* Wordy comments: 
	 * zone is used to combine resources from sveral plugins 
	 * into one domain. Only in default domain there are several
	 * zones. For domains generated by plugin, one zone in 
	 * one domain.
	 */
	struct list_head zone_list;	/* zone list in the domain */
	struct list_head session_list;	/* session list in the domain*/
};

struct oh_zone {
	struct list_head node;		/* link-node in zone list of domain */

	struct oh_domain *domain; 	/* pointer to the container doamin */
		
	/* Because resource id is assigned in domain wide, so we should
	 * refer domain's res_counter for resource id */
	struct list_head res_list;	/*res list in the zone */
