...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SWITCH_BLADE,32}{PROCESSOR_BOARD,51}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_PROCESSOR_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 51) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 51);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 32) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 32);
	    return -1;
	}
