...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SHELF_MANAGER,5}{FRONT_PANEL_BOARD,73}";
        ...
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_SHELF_MANAGER) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_SHELF_MANAGER);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 5) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 5);
	    return -1;
	}
