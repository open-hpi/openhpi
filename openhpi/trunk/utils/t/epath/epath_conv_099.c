...
{
        char *test_string = "{EXTERNAL_ENVIRONMENT,90}{BIOS,65}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_EXTERNAL_ENVIRONMENT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_EXTERNAL_ENVIRONMENT);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 90) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 90);
	    return -1;
	}
