...
{
        char *test_string = "{OTHER,0}{FRONT_PANEL_BOARD,49}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 49) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 49);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_OTHER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_OTHER);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 0) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
