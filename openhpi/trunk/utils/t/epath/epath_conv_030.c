...
{
        char *test_string = "{POWER_SUPPLY,93}{PHYSICAL_SLOT,89}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_PHYSICAL_SLOT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_PHYSICAL_SLOT);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 89) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 89);
	    return -1;
	}
