...
{
        char *test_string = "{PROCESSOR,4}{MEMORY_MODULE,30}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 30) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 30);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_PROCESSOR) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_PROCESSOR);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 4) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 4);
	    return -1;
	}
