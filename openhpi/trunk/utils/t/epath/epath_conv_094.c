...
{
        char *test_string = "{SWITCH_BLADE,97}{DISPLAY_PANEL,4}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DISPLAY_PANEL) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_DISPLAY_PANEL);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 4) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 4);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SWITCH_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SWITCH_BLADE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 97) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 97);
	    return -1;
	}
