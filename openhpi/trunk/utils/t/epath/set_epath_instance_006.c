...
/**
 * set_epath_instance test6.
 *   call set_epath_instance with entity path that has 4 elements, victim element in the middle
...
int main(int argc, char **argv)
{
        SaHpiEntityPathT ep = {{{SAHPI_ENT_FAN, 11099},
                                ...
                                {0}}};
        SaHpiEntityInstanceT x = 123456;
        int mydebug = 0;
         
        if (mydebug) printf(" test6\n");
        if(set_epath_instance(&ep, SAHPI_ENT_RACK, x)) {
        ...
        if (ep.Entry[0].EntityInstance != 11099) {
        ...
        if (ep.Entry[1].EntityInstance != 2002) {
        ...
        if (ep.Entry[3].EntityInstance != 440044) {
