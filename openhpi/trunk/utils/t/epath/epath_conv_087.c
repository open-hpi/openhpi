...
{
        char *test_string = "{COOLING_UNIT,57}{DISK_DRIVE_BAY,31}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DISK_DRIVE_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_DISK_DRIVE_BAY);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 31) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 31);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 57) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
