...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{OPERATING_SYSTEM,13}{IO_BLADE,67}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_IO_BLADE) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_IO_BLADE);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 67) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 67);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_OPERATING_SYSTEM) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_OPERATING_SYSTEM);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 13) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 13);
	    return -1;
	}
