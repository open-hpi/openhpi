...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{UNSPECIFIED,72}{OTHER,21}";
        ...
        if (tmp_ep.Entry[0].EntityLocation != 21) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 21);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 72) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 72);
	    return -1;
	}
