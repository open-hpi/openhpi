...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{DISK_BLADE,18}{UNSPECIFIED,31}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_UNSPECIFIED);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 31) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 31);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_DISK_BLADE) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_DISK_BLADE);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 18) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 18);
	    return -1;
	}
