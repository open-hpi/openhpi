...
{
        char *test_string = "{OEM_SYSINT_SPECIFIC,49}{EXTERNAL_ENVIRONMENT,18}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 18) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 18);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_OEM_SYSINT_SPECIFIC) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_OEM_SYSINT_SPECIFIC);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 49) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
