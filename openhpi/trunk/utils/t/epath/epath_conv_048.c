...
{
        char *test_string = "{SYSTEM_BOARD,88}{COOLING_UNIT,27}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_COOLING_UNIT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_COOLING_UNIT);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYSTEM_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SYSTEM_BOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 88) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 88);
	    return -1;
	}
