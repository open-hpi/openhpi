...
{
        char *test_string = "{FAN,98}{RACK_MOUNTED_SERVER,5}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_RACK_MOUNTED_SERVER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_RACK_MOUNTED_SERVER);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_FAN) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
