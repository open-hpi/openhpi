...
{
        char *test_string = "{DISPLAY_PANEL,1}{SWITCH_BLADE,44}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SWITCH_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SWITCH_BLADE);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 1) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 1);
	    return -1;
	}
