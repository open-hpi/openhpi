...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{FRONT_PANEL_BOARD,81}{POWER_SYSTEM_BOARD,93}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_POWER_SYSTEM_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 93) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 93);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_FRONT_PANEL_BOARD) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_FRONT_PANEL_BOARD);
	    return -1;
	}
