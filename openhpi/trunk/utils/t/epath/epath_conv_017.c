...
{
        char *test_string = "{SBC_SUBBOARD,98}{ADVANCEDTCA_CHASSIS,7}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 7) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 7);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SBC_SUBBOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SBC_SUBBOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 98) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 98);
	    return -1;
	}
