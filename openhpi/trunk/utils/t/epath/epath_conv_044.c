...
{
        char *test_string = "{SYS_MGMNT_MODULE,77}{BOARD_SET_SPECIFIC,84}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYS_MGMNT_MODULE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SYS_MGMNT_MODULE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 77) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 77);
	    return -1;
	}
