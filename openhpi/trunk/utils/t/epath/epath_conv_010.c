...
{
        char *test_string = "{DISK_BLADE,66}{POWER_UNIT,62}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_POWER_UNIT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_POWER_UNIT);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 62) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 62);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_DISK_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_DISK_BLADE);
	    return -1;
	}
