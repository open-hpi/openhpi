...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{BACK_PANEL_BOARD,44}{BACK_PANEL_BOARD,97}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_BACK_PANEL_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 97) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 97);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 44) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 44);
	    return -1;
	}
