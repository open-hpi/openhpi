...
{
        char *test_string = "{IO_BLADE,47}{FRONT_PANEL_BOARD,34}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_FRONT_PANEL_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_FRONT_PANEL_BOARD);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 34) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 34);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_IO_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_IO_BLADE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 47) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 47);
	    return -1;
	}
