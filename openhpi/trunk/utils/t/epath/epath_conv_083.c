...
{
        char *test_string = "{BIOS,17}{PERIPHERAL_BAY,62}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_BIOS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_BIOS);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 17) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 17);
	    return -1;
	}
