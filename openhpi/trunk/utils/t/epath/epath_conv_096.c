...
{
        char *test_string = "{POWER_DISTRIBUTION_UNIT,56}{BATTERY,69}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 69) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 69);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_POWER_DISTRIBUTION_UNIT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_POWER_DISTRIBUTION_UNIT);
	    return -1;
	}
