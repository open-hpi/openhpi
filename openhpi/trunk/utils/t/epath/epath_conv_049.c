...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{OTHER_SYSTEM_BOARD,46}{POWER_DISTRIBUTION_UNIT,59}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_POWER_DISTRIBUTION_UNIT) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_POWER_DISTRIBUTION_UNIT);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 59) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 59);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_OTHER_SYSTEM_BOARD) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_OTHER_SYSTEM_BOARD);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 46) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 46);
	    return -1;
	}
