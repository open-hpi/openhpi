...
{
        char *test_string = "{MEMORY_DEVICE,37}{POWER_SYSTEM_BOARD,46}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_POWER_SYSTEM_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_POWER_SYSTEM_BOARD);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 46) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 46);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_MEMORY_DEVICE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_MEMORY_DEVICE);
	    return -1;
	}
