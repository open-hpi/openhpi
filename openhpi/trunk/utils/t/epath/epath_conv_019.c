...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SYS_EXPANSION_BOARD,37}{ALARM_MANAGER,69}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_ALARM_MANAGER);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 69) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 69);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_SYS_EXPANSION_BOARD) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_SYS_EXPANSION_BOARD);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 37) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 37);
	    return -1;
	}
