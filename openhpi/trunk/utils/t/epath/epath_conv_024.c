...
{
        char *test_string = "{MEMORY_MODULE,56}{OTHER_SYSTEM_BOARD,77}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_OTHER_SYSTEM_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_OTHER_SYSTEM_BOARD);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 56) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 56);
	    return -1;
	}
