...
{
        char *test_string = "{POWER_SUPPLY,51}{MEMORY_DEVICE,27}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_MEMORY_DEVICE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_MEMORY_DEVICE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 27) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 27);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 51) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 51);
	    return -1;
	}
