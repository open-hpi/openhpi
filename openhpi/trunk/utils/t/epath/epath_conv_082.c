...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{OTHER_SYSTEM_BOARD,57}{OTHER_CHASSIS_BOARD,24}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_OTHER_CHASSIS_BOARD) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_OTHER_CHASSIS_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 24) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 24);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 57) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 57);
	    return -1;
	}
