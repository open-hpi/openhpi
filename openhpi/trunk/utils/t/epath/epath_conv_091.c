...
{
        char *test_string = "{SYSTEM_BLADE,85}{BOARD_SET_SPECIFIC,41}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 41) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 41);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 85) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 85);
	    return -1;
	}
