...
{
        char *test_string = "{ADD_IN_CARD,2}{POWER_MODULE,72}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_POWER_MODULE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_POWER_MODULE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 72) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 72);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 2) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
