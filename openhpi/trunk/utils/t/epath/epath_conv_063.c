...
{
        char *test_string = "{DRIVE_BACKPLANE,32}{POWER_SUPPLY,51}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_POWER_SUPPLY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_POWER_SUPPLY);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 51) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 51);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_DRIVE_BACKPLANE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
