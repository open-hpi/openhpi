...
{
        char *test_string = "{SYSTEM_CHASSIS,92}{SHELF_MANAGER,66}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SHELF_MANAGER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SHELF_MANAGER);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 66) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 66);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYSTEM_CHASSIS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SYSTEM_CHASSIS);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 92) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 92);
	    return -1;
	}
