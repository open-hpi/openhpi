...
{
        char *test_string = "{POWER_SYSTEM_BOARD,21}{BIOS,80}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_POWER_SYSTEM_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_POWER_SYSTEM_BOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 21) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 21);
	    return -1;
	}
