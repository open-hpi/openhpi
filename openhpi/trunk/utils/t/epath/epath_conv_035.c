...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{IO_SUBBOARD,48}{GROUP,35}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_GROUP) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_GROUP);
	    return -1;
	}
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_IO_SUBBOARD);
	    return -1;
	}
