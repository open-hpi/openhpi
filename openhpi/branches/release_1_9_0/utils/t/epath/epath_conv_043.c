...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SUBBOARD_CARRIER_BLADE,99}{PHYSICAL_SLOT,69}";
        ...
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_SUBBOARD_CARRIER_BLADE) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_SUBBOARD_CARRIER_BLADE);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 99) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 99);
	    return -1;
	}
