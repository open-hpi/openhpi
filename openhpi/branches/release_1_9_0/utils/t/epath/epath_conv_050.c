...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SYSTEM_BLADE,50}{SYSTEM_BUS,64}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_SYSTEM_BUS) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_SYSTEM_BUS);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 64) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 64);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_SYSTEM_BLADE) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_SYSTEM_BLADE);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 50) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 50);
	    return -1;
	}
