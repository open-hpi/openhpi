...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SPEC_PROC_BLADE,80}{RACK_MOUNTED_SERVER,83}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_RACK_MOUNTED_SERVER) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_RACK_MOUNTED_SERVER);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 83) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 83);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 80) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 80);
	    return -1;
	}
