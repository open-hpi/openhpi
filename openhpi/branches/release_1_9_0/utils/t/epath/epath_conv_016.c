...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{DISK_DRIVE_BAY,88}{OTHER_SYSTEM_BOARD,46}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_OTHER_SYSTEM_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 46) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 46);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_DISK_DRIVE_BAY) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_DISK_DRIVE_BAY);
	    return -1;
	}
