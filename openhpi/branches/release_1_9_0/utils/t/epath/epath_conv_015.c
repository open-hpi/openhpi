...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{UNKNOWN,22}{ALARM_MANAGER,10}";
        ...
        if (tmp_ep.Entry[0].EntityLocation != 10) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 10);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 22) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 22);
	    return -1;
	}
