...
{
        char *test_string = "{SYSTEM_BLADE,95}{SUBBOARD_CARRIER_BLADE,37}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SUBBOARD_CARRIER_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SUBBOARD_CARRIER_BLADE);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYSTEM_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
