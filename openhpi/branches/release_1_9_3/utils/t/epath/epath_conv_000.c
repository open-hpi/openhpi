...
{
        char *test_string = "{PROCESSOR_MODULE,95}{ADVANCEDTCA_CHASSIS,61}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_ADVANCEDTCA_CHASSIS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_ADVANCEDTCA_CHASSIS);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 61) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 61);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_PROCESSOR_MODULE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_PROCESSOR_MODULE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 95) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 95);
	    return -1;
	}
