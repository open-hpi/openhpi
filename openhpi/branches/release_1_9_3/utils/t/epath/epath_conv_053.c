...
{
        char *test_string = "{RACK_MOUNTED_SERVER,66}{PROCESSOR_MODULE,10}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_PROCESSOR_MODULE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_PROCESSOR_MODULE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 10) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 10);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_RACK_MOUNTED_SERVER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_RACK_MOUNTED_SERVER);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 66) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 66);
	    return -1;
	}
