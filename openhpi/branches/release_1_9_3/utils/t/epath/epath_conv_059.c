...
{
        char *test_string = "{ADVANCEDTCA_CHASSIS,93}{SYS_MGMNT_SOFTWARE,85}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 85) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 85);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_ADVANCEDTCA_CHASSIS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_ADVANCEDTCA_CHASSIS);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 93) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 93);
	    return -1;
	}
