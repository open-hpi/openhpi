...
{
        char *test_string = "{UNSPECIFIED,28}{CHASSIS_BACK_PANEL_BOARD,23}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_CHASSIS_BACK_PANEL_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_UNSPECIFIED) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_UNSPECIFIED);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 28) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 28);
	    return -1;
	}
