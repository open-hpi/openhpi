...
{
        char *test_string = "{INTERCONNECT,37}{ADD_IN_CARD,60}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 60) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 60);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_INTERCONNECT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_INTERCONNECT);
	    return -1;
	}
