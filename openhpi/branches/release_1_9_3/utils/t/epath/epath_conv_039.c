...
{
        char *test_string = "{BACK_PANEL_BOARD,37}{PERIPHERAL_BAY,84}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_PERIPHERAL_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_PERIPHERAL_BAY);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_BACK_PANEL_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_BACK_PANEL_BOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 37) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 37);
	    return -1;
	}
