...
{
        char *test_string = "{IO_SUBBOARD,24}{CHASSIS_SPECIFIC,71}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_CHASSIS_SPECIFIC) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_IO_SUBBOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_IO_SUBBOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 24) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 24);
	    return -1;
	}
