...
{
        char *test_string = "{SYS_MGMNT_SOFTWARE,15}{MEMORY_MODULE,28}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_MEMORY_MODULE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_MEMORY_MODULE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 28) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 28);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYS_MGMNT_SOFTWARE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SYS_MGMNT_SOFTWARE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 15) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 15);
	    return -1;
	}
