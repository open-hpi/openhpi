...
{
        char *test_string = "{DISK_DRIVE_BAY,32}{DRIVE_BACKPLANE,50}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DRIVE_BACKPLANE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_DRIVE_BACKPLANE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 50) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 50);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 32) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 32);
	    return -1;
	}
