...
{
        char *test_string = "{POWER_UNIT,72}{SHELF_MANAGER,44}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 44) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 44);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_POWER_UNIT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_POWER_UNIT);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 72) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 72);
	    return -1;
	}
