...
{
        char *test_string = "{POWER_SUPPLY,80}{POWER_DISTRIBUTION_UNIT,23}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_POWER_SUPPLY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_POWER_SUPPLY);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 80) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
