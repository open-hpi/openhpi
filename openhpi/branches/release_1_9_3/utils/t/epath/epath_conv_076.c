...
{
        char *test_string = "{ALARM_MANAGER,46}{OEM_SYSINT_SPECIFIC,84}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_OEM_SYSINT_SPECIFIC) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_OEM_SYSINT_SPECIFIC);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 84) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 84);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_ALARM_MANAGER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_ALARM_MANAGER);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 46) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 46);
	    return -1;
	}
