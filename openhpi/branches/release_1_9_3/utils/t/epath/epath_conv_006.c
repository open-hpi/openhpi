...
{
        char *test_string = "{BACK_PANEL_BOARD,61}{SYSTEM_BUS,99}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SYSTEM_BUS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SYSTEM_BUS);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 99) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityLocation != 61) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 61);
	    return -1;
	}
