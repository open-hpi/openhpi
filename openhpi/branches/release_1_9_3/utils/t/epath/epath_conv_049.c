...
{
        char *test_string = "{IO_BLADE,89}{PHYSICAL_SLOT,94}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 94) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 94);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 89) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 89);
	    return -1;
	}
