...
/**
 * main: Starts with an RPTable of 10 resources, adds 5 annunciator rdr
 * to first resource. Fetches annunciators ++randomly by record id and compares
...
int main(int argc, char **argv)
{
        ...
        for (i = 0; i < 5; i++) {
                if (oh_add_rdr(rptable, SAHPI_FIRST_ENTRY, annunciators + i, NULL,0))
                        return 1;
                else
                        records = g_slist_append(records, annunciators + i);
        }
