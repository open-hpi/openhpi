...
static int inject_event(char *plugin_name) {
    ...
    *txtptr = '\0';
    ...
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_TYPE, SAHPI_CTRL_TYPE_OEM);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    ...
    sprintf(txtptr, "%s=%d", SIM_MSG_RDR_CTRL_OEM_MID, 123);
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    }
    sprintf(txtptr, "%s=%s", SIM_MSG_RDR_CTRL_OEM_CONFIG, "My config data");
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
    }
    sprintf(txtptr, "%s=%s", SIM_MSG_RDR_CTRL_OEM_BODY, "My config body");
    n += strlen(txtptr) + 1;
    txtptr = buf.mtext + n;
    if (n > SIM_MSG_QUEUE_BUFSIZE) {
        return -1;
