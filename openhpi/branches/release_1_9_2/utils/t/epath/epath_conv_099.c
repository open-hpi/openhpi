...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{FAN,71}{SBC_BLADE,92}";
        ...
        if (tmp_ep.Entry[0].EntityLocation != 92) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 92);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_FAN) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_FAN);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 71) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 71);
	    return -1;
	}
