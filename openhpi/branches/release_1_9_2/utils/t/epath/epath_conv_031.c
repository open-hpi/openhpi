...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{FRONT_PANEL_BOARD,48}{INTERCONNECT,38}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_INTERCONNECT);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 38) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 38);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 48) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 48);
	    return -1;
	}
