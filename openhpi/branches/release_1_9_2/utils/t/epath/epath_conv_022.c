...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{DISPLAY_PANEL,74}{POWER_UNIT,66}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_POWER_UNIT) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_POWER_UNIT);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_DISPLAY_PANEL) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_DISPLAY_PANEL);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 74) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 74);
	    return -1;
	}
