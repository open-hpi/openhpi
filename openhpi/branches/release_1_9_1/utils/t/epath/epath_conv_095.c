...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SBC_SUBBOARD,73}{POWER_SUPPLY,37}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_POWER_SUPPLY);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 37) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 37);
	    return -1;
	}
