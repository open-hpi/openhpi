...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{CHASSIS_BACK_PANEL_BOARD,64}{UNSPECIFIED,72}";
        ...
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_CHASSIS_BACK_PANEL_BOARD) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_CHASSIS_BACK_PANEL_BOARD);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 64) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 64);
	    return -1;
	}
