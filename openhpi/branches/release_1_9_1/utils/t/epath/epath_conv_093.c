...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SUBRACK,37}{OPERATING_SYSTEM,53}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_OPERATING_SYSTEM) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_OPERATING_SYSTEM);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 53) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 53);
	    return -1;
	}
