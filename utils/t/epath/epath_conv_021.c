...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{UNKNOWN,70}{SYSTEM_CHASSIS,62}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_SYSTEM_CHASSIS) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_SYSTEM_CHASSIS);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 62) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 62);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_UNKNOWN) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_UNKNOWN);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 70) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 70);
	    return -1;
	}
