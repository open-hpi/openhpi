...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{CHASSIS_SPECIFIC,60}{EXTERNAL_ENVIRONMENT,18}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_EXTERNAL_ENVIRONMENT);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 18) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 18);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 60) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 60);
	    return -1;
	}
