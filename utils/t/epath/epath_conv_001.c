...
{
        char *test_string = "{OPERATING_SYSTEM,0}{GROUP,67}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 67) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 67);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_OPERATING_SYSTEM) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
