...
{
        char *test_string = "{SBC_BLADE,41}{OTHER_CHASSIS_BOARD,23}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_OTHER_CHASSIS_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_OTHER_CHASSIS_BOARD);
	    return -1;
	}
