...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{BACK_PANEL_BOARD,35}{SBC_SUBBOARD,74}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_SBC_SUBBOARD) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_SBC_SUBBOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 74) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 74);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_BACK_PANEL_BOARD) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_BACK_PANEL_BOARD);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 35) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 35);
	    return -1;
	}
