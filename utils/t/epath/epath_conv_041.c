...
{
        char *test_string = "{PERIPHERAL_BAY,63}{IO_SUBBOARD,34}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_IO_SUBBOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_PERIPHERAL_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_PERIPHERAL_BAY);
	    return -1;
	}
