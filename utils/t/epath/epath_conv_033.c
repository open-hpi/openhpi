...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{BOARD_SET_SPECIFIC,97}{REMOTE,70}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_REMOTE) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_REMOTE);
	    return -1;
	}
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 97);
	    return -1;
	}
