...
{
        char *test_string = "{POWER_MGMNT,25}{SUB_CHASSIS,80}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 80) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 80);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_POWER_MGMNT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_POWER_MGMNT);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 25) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 25);
	    return -1;
	}
