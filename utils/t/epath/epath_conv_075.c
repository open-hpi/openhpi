...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{POWER_MODULE,42}{ADD_IN_CARD,75}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if(tmp_ep.Entry[0].EntityType != SAHPI_ENT_ADD_IN_CARD)
                return 1;
                
        if(tmp_ep.Entry[0].EntityInstance != 75)
                return 1;
        
        if(tmp_ep.Entry[1].EntityType != SAHPI_ENT_POWER_MODULE)
                return 1;
        
        if(tmp_ep.Entry[1].EntityInstance != 42)
                return 1;
