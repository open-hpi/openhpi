...
{
        char *test_string = "{COOLING_UNIT,59}{ADVANCEDTCA_CHASSIS,56}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 56) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 56);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_COOLING_UNIT) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_COOLING_UNIT);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 59) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 59);
	    return -1;
	}
