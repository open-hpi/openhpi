...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{COMPACTPCI_CHASSIS,43}{CHASSIS_SPECIFIC,35}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_CHASSIS_SPECIFIC) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_CHASSIS_SPECIFIC);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 35) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 35);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_COMPACTPCI_CHASSIS) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_COMPACTPCI_CHASSIS);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 43) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 43);
	    return -1;
	}
