...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{SBC_BLADE,87}{CHASSIS_BACK_PANEL_BOARD,80}";
	    ...
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_CHASSIS_BACK_PANEL_BOARD);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 80) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 80);
	    return -1;
	}
        ...
        if (tmp_ep.Entry[1].EntityLocation != 87) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 87);
	    return -1;
	}
