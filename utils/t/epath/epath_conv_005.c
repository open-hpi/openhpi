...
{
        char *test_string = "{DISK_DRIVE_BAY,63}{SYS_MGMNT_SOFTWARE,57}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 57) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 57);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_DISK_DRIVE_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_DISK_DRIVE_BAY);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 63) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 63);
	    return -1;
	}
