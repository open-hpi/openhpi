...
{
        char *test_string = "{DISK_DRIVE,8}{PROCESSOR_MODULE,15}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 15) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 15);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_DISK_DRIVE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_DISK_DRIVE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 8) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 8);
	    return -1;
	}
