...
{
        char *test_string = "{CHASSIS_SPECIFIC,23}{FRONT_PANEL_BOARD,16}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 16) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 16);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_CHASSIS_SPECIFIC) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_CHASSIS_SPECIFIC);
	    return -1;
	}
