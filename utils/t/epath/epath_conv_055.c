...
{
        char *test_string = "{SYSTEM_BUS,23}{SYS_MGMNT_SOFTWARE,23}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 23) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 23);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SYSTEM_BUS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SYSTEM_BUS);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 23) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 23);
	    return -1;
	}
