...
/**
 * set_epath_instance test3.
 *   call set_epath_instance with entity path that has 2 elements
...
int main(int argc, char **argv)
{
        ...
        SaHpiEntityInstanceT x = 777;
        int mydebug = 0;
         
        if (mydebug) printf(" test3\n");
        if(set_epath_instance(&ep, SAHPI_ENT_UNKNOWN, x)) {
        ...
        if (ep.Entry[0].EntityInstance != 1) {
