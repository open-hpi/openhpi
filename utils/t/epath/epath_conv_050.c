...
{
        char *test_string = "{SYSTEM_BUS,66}{ALARM_MANAGER,43}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_ALARM_MANAGER) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_ALARM_MANAGER);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 43) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 43);
	    return -1;
	}
