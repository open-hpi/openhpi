...
{
        char *test_string = "{SWITCH,3}{SWITCH,3}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SWITCH) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SWITCH);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 3) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 3);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SWITCH) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SWITCH);
	    return -1;
	}
