...
{
        char *test_string = "{SYSTEM_CHASSIS,27}{DISK_BLADE,96}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DISK_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_DISK_BLADE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 96) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 96);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 27) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 27);
	    return -1;
	}
