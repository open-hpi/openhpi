...
/**
 * set_epath_instance test8.
 *   call set_epath_instance with 4 element entity path, victim element at head 
...
int main(int argc, char **argv)
{
        SaHpiEntityPathT ep = {{{SAHPI_ENT_ADD_IN_CARD, 101},
                                ...
                                {0}}};
        SaHpiEntityInstanceT x = 555555;
        int mydebug = 0;
         
        if (mydebug) printf(" test8\n");
        if(set_epath_instance(&ep, SAHPI_ENT_ADD_IN_CARD, x)) {
        ...
        if (ep.Entry[1].EntityInstance != 2020) {
        ...
        if (ep.Entry[2].EntityInstance != 30303) {
        ...
        if (ep.Entry[3].EntityInstance != 404040) {
