...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{BOARD_SET_SPECIFIC,82}{ADVANCEDTCA_CHASSIS,13}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_ADVANCEDTCA_CHASSIS) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_ADVANCEDTCA_CHASSIS);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 13) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 13);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityType != SAHPI_ENT_BOARD_SET_SPECIFIC) {
	    printf("ERROR! Testcase failed Entry[1].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityType, SAHPI_ENT_BOARD_SET_SPECIFIC);
	    return -1;
	}
        
        if (tmp_ep.Entry[1].EntityLocation != 82) {
	    printf("ERROR! Testcase failed Entry[1].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[1].EntityLocation, 82);
	    return -1;
	}
