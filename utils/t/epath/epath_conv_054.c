...
{
        char *test_string = "{PERIPHERAL_BAY_2,60}{SPEC_PROC_BLADE,65}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SPEC_PROC_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SPEC_PROC_BLADE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 65) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 65);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_PERIPHERAL_BAY_2) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_PERIPHERAL_BAY_2);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 60) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 60);
	    return -1;
	}
