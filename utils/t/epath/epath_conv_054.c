...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{PERIPHERAL_BAY,30}{FAN,65}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if(tmp_ep.Entry[0].EntityType != SAHPI_ENT_FAN)
                return 1;
                
        if(tmp_ep.Entry[0].EntityInstance != 65)
                return 1;
        
        if(tmp_ep.Entry[1].EntityType != SAHPI_ENT_PERIPHERAL_BAY)
                return 1;
        
        if(tmp_ep.Entry[1].EntityInstance != 30)
                return 1;
