...
{
        char *test_string = "{PROCESSOR_BOARD,62}{OPERATING_SYSTEM,70}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_OPERATING_SYSTEM) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_OPERATING_SYSTEM);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 70) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 70);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_PROCESSOR_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_PROCESSOR_BOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 62) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 62);
	    return -1;
	}
