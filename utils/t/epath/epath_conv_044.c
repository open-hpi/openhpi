...
int main(int argc, char **argv) 
{
        ...
        char *entity_root = "{IO_BLADE,50}{DISK_BAY,89}";
        
        string2entitypath(entity_root, &tmp_ep);
         
        if (tmp_ep.Entry[0].EntityType != SAHPI_ENT_DISK_BAY) {
	    printf("ERROR! Testcase failed Entry[0].EntityType compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityType, SAHPI_ENT_DISK_BAY);
	    return -1;
	}
                
        if (tmp_ep.Entry[0].EntityLocation != 89) {
	    printf("ERROR! Testcase failed Entry[0].EntityLocation compare\n");
	    printf("Received=%d; Expected=%d\n", tmp_ep.Entry[0].EntityLocation, 89);
	    return -1;
	}
