...
{
        char *test_string = "{SYSTEM_BUS,84}{SBC_SUBBOARD,65}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SBC_SUBBOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SBC_SUBBOARD);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 84) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 84);
	    return -1;
	}
