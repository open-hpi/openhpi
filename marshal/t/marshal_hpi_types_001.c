...
main( int argc, char *argv[] )
{
  ...
  // use 5 records
  unsigned char *b = buffer1 + sizeof( SaHpiInventoryDataT ) + 5 * sizeof( SaHpiInventDataRecordT * );
  ...
  g->ProductName = 0;
  b = text_buffer( b, &g->ProductVersion, SAHPI_TL_TYPE_ASCII6, SAHPI_LANG_LATIN, 7, "abc" );
  b = text_buffer( b, &g->ModelNumber, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_MALAY, 7, "tttt" );
  b = text_buffer( b, &g->SerialNumber, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_NORWEGIAN, 7, "durban" );
  b = text_buffer( b, &g->PartNumber, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_FRENCH, 7, "luxor" );
  b = text_buffer( b, &g->FileId, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_HAUSA, 7, "maun" );
  b = text_buffer( b, &g->AssetTag, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_AYMARA, 7, "sydney" );
  g->CustomField[0] = 0;
  ...
  b = text_buffer( b, &g->Manufacturer, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_SERBIAN, 7, "belgrad" );
  g->ProductName = 0;
  b = text_buffer( b, &g->ProductVersion, SAHPI_TL_TYPE_ASCII6, SAHPI_LANG_LATIN, 7, "rom" );
  b = text_buffer( b, &g->ModelNumber, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_MALAY, 7, "1" );
  b = text_buffer( b, &g->SerialNumber, SAHPI_TL_TYPE_BINARY, SAHPI_LANG_NORWEGIAN, 7, "aaaa" );
