				  ...
				  unsigned char *b )
{
  ...
  tUint32 *num;
  num = malloc(sizeof(tUint32 *));
  memcpy(&num, b, sizeof(num));
				...
				const void *buffer, void *user_data )
{
  ...
  for( i = 0; i < num; i++ )
	...
	memcpy(id->DataRecords[i], data_ptr, sizeof(id->DataRecords[i]));
